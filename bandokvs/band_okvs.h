...
inline bool DebugEnabled() {
  static const bool enabled = []() {
    const char* v0 = std::getenv("BANDOKVS_DEBUG");
    const char* v1 = std::getenv("ONEMALPSU_BANDOKVS_DEBUG");
    return (v0 != nullptr && std::string(v0) != "0") ||
           (v1 != nullptr && std::string(v1) != "0");
  }();
  return enabled;
}
...
inline void DebugLog(const std::string& msg) {
  if (DebugEnabled()) {
    std::cerr << "[bandokvs] " << msg << std::endl;
  }
}
...
class ScopedTimer {
 public:
  ...
  ~ScopedTimer() {
    ...
    const double sec = std::chrono::duration<double>(end - start_).count();
    std::cerr << "[bandokvs] " << label_ << " took " << sec << " s"
              << std::endl;
  }
...
template <typename T>
inline T CreateBand(oc::span<oc::block> blocks, __uint128_t high_mask) {
  return T();
}

template <>
inline uint<1> CreateBand(oc::span<oc::block> blocks, __uint128_t high_mask) {
  return uint<1>(((blocks[0]).get<__uint128_t>(0) & high_mask) | 1);
}

template <>
inline uint<2> CreateBand(oc::span<oc::block> blocks, __uint128_t high_mask) {
  return uint<2>((blocks[0]).get<__uint128_t>(0) | 1,
                 (blocks[1]).get<__uint128_t>(0) & high_mask);
}

template <>
inline uint<3> CreateBand(oc::span<oc::block> blocks, __uint128_t high_mask) {
...
template <>
inline uint<4> CreateBand(oc::span<oc::block> blocks, __uint128_t high_mask) {
...
template <>
inline uint<5> CreateBand(oc::span<oc::block> blocks, __uint128_t high_mask) {
  return uint<5>(
      (blocks[0]).get<__uint128_t>(0) | 1, (blocks[1]).get<__uint128_t>(0),
      (blocks[2]).get<__uint128_t>(0), (blocks[3]).get<__uint128_t>(0),
      (blocks[4]).get<__uint128_t>(0) & high_mask);
}

template <>
inline uint<6> CreateBand(oc::span<oc::block> blocks, __uint128_t high_mask) {
  return uint<6>(
      (blocks[0]).get<__uint128_t>(0) | 1, (blocks[1]).get<__uint128_t>(0),
      (blocks[2]).get<__uint128_t>(0), (blocks[3]).get<__uint128_t>(0),
...
template <typename T>
inline void GenBands(int n, const oc::block* keys, int okvs_length,
                     int band_length, Band<T>* out) {
...
inline void GenBandsAndValues(int n, const oc::block* keys, const V* values,
...
class BandOkvs {
 public:
  void Init(int n, int m, int band_length,
  ...
  int Size() const { return num_vars_ + 64; }

  int NumVars() const { return num_vars_; }

  int NumEqns() const { return num_eqns_; }
 ...
 private:
  ...
  inline bool ReduceToRowEchelonHelper(const BandAndValue<T, V>* bands,
                                       T* reduced_matrix, V* reduced_values) {
    int n = num_eqns_;
    for (int i = 0; i < n; ++i) {
  ...
  ReduceToRowEchelon(const BandAndValue<T, V>* bands, T* reduced_matrix,
                     V* reduced_values) {
    return ReduceToRowEchelonHelper<T, V>(bands, reduced_matrix,
                                          reduced_values);
  }
  ...
  template <typename T, typename V>
  inline
      typename std::enable_if<not std::is_same<V, oc::block>::value, bool>::type
      ReduceToRowEchelon(const BandAndValue<T, V>* bands, T* reduced_matrix,
                         V* reduced_values) {
    return ReduceToRowEchelonHelper<T, V>(bands, reduced_matrix, reduced_values,
                                          0);
  }

  template <typename T, typename V>
  bool EncodeHelper(const BandAndValue<T, V>* bands, V* out) {
    ScopedTimer timer("Encode::ReduceToRowEchelon+Solve");
    std::vector<T> reduced_matrix(num_vars_);

    {
      ScopedTimer stage("Encode::ReduceToRowEchelon");
    ...
    {
      ScopedTimer stage("Encode::Solve");
  ...
  bool EncodeHelper(const oc::block* keys, const V* values, V* out) {
    DebugLog("Encode start: n=" + std::to_string(num_eqns_) +
             ", m=" + std::to_string(num_vars_) +
             ", band_length=" + std::to_string(band_length_));
    const int n = num_eqns_;
    {
      ScopedTimer stage("Encode::PRNGInit");
    ...
    {
      ScopedTimer stage("Encode::GenBandsAndValues");
      GenBandsAndValues<T, V>(n, keys, values, num_vars_, band_length_, bands);
    }
    {
      ScopedTimer stage("Encode::Sort");
    ...
    bool encoded = false;
    {
      ScopedTimer stage("Encode::Core");
      encoded = EncodeHelper<T, V>(bands, out);
    }
    DebugLog(std::string("Encode finished: encoded=") +
             (encoded ? "true" : "false"));
    return encoded;
  }
  ...
  void DecodeHelper(const oc::block* keys, const V* okvs, V* out) {
    DebugLog("Decode start: n=" + std::to_string(num_eqns_) +
             ", m=" + std::to_string(num_vars_) +
             ", band_length=" + std::to_string(band_length_));
    int n = num_eqns_;
    Band<T>* bands = (Band<T>*)malloc(n * sizeof(Band<T>));
    {
      ScopedTimer stage("Decode::GenBands");
      GenBands<T>(n, keys, num_vars_, band_length_, bands);
    }
    {
      ScopedTimer stage("Decode::Sort");
      boost::sort::spreadsort::integer_sort(bands, bands + n, rightshift(),
                                            lessthan());
    }
    {
      ScopedTimer stage("Decode::Apply");
      DecodeHelper(bands, okvs, out);
    }
    DebugLog("Decode finished");
  }
...
#pragma GCC unroll 8
    for (int j = 0, k = 0; j < length; j += 8, k++) {
      ...
      res = _mm512_xor_epi64(
          res, _mm512_maskz_loadu_epi64(mask2, &reduced_values[i + j + 4]));
    }
    return res;
  }
