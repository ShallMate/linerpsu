...
std::vector<uint128_t> PSUSend(const std::shared_ptr<yacl::link::Context>& ctx,
             std::vector<uint128_t>& elem_hashes,CuckooHash& T_X,uint32_t cuckoolen,okvs::Baxos baxos,okvs::Baxos baxos2) {
...
std::vector<uint128_t> PSURecv(const std::shared_ptr<yacl::link::Context>& ctx,
                          std::vector<uint128_t>& elem_hashes,uint32_t cuckoolen,okvs::Baxos baxos,okvs::Baxos baxos2) {
  ...
  for (int64_t idx = begin; idx < end; ++idx) {
        __m128i x_block = _mm_loadu_si128(reinterpret_cast<const __m128i*>(&elem_hashes[idx]));
        size_t idx1 = idx * 3;
  ...
  yacl::parallel_for(0, n, [&](int64_t begin, int64_t end) {
    for (int64_t idx = begin; idx < end; ++idx) {
      sendermasks[idx] = RS[idx]^((sendermasks[idx] & suint) ^ all_C[idx] ^ omega);
    }
  });
  uint128_t okvssize2 = baxos2.size();
  std::vector<uint128_t> pp(okvssize2);
