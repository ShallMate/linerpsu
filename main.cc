...
void OurPSURR22() {
  ...
  baxos.Init(cuckoolen, bin_size, weight, ssp, okvs::PaxosParam::DenseType::GF128,
             seed);

  baxos2.Init(nr*3, bin_size*3, weight, ssp, okvs::PaxosParam::DenseType::GF128,
             seed);
  ...
  std::future<std::vector<__uint128_t>> sender = std::async(std::launch::async, [&] {
      return psu::PSUSend(lctxs[0], items_a,T_X,cuckoolen, baxos, baxos2);
  });

  std::future<std::vector<__uint128_t>> receiver = std::async(std::launch::async, [&] {
  ...
  std::future<std::vector<__uint128_t>> pssender = std::async(std::launch::async, [&] {
      return ps::PSSend(lctxs[0], pi, fxs);
  });

  std::future<std::vector<__uint128_t>> psreceiver = std::async(std::launch::async, [&] {
      return ps::PSRecv(lctxs[1], fxr, rr, k);
  });
  ...
  std::future<std::vector<__uint128_t>> eqotereceiver = std::async(std::launch::async, [&] {
    return eqote::EQOTERecv(lctxs[1], eqr);
  });
  eqotesender.get();
