...
OUT_DIR="${LINERPSU_OUT_DIR:-${ROOT}/examples/linerpsu/results/Ours/psu_okvs_$(date +%Y%m%d_%H%M%S)}"
...
printf 'backend,backend_label,logn,ns,nr,cuckoolen,status,offline_s,online_s,total_s,total_comm_bytes,total_comm_mb,result_size,console_log\n' > "${SUMMARY}"
...
run_case() {
  ...
  local total_comm="" total_comm_mb="" result_size=""
  ...
  printf '%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n' \
    ...
    "${status}" "${offline_s}" "${online_s}" "${total_s}" \
    "${total_comm}" "${total_comm_mb}" "${result_size}" "${log_file}" >> "${SUMMARY}"
...

{
  ...
  for backend in ${BACKENDS}; do
    ...
    for logn in ${LOGNS}; do
      printf ',%s' "$(lookup_summary "${backend}" "${logn}" total_s)"
    done
    printf '\n'

    printf '%s,Communication (MB)' "${label}"
    for logn in ${LOGNS}; do
      printf ',%s' "$(lookup_summary "${backend}" "${logn}" total_comm_mb)"
    done
    printf '\n'
  done
} > "${TABLE}"
